...
from phishin_train_ffnn import LightningFFNN, URLDataset
...
def evaluate_ffnn():
    ...
    model = LightningFFNN.load_from_checkpoint("models/ffnn_lightning.ckpt")
    model.eval()
    ...
    with torch.no_grad():
        for x, y in test_loader:
            probs = torch.sigmoid(model(x))  # convert logits to probabilities
            preds = (probs > 0.5).int()
